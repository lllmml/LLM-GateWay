export function CostNotice() {
  return (
    <p className="rounded-md border border-slate-200 bg-slate-50 px-4 py-3 text-xs leading-5 text-slate-500">
      ...
      not invoices. Requests without an effective price version or usage contribute to counts but not to cost. In Week 7
      DeepSeek requests record usage but carry no cost: its cache hit/miss and peak/off-peak billing dimensions are not yet
      persisted (ADR-016).
    </p>
  );
}
