...
| HTTP status classification | 401 (invalid upstream credential) and 402 (billing) -> `provider_unavailable` (server-side access problem, never a client HTTP 400); 429 -> `provider_rate_limited`; 408/504 -> `provider_timeout`; other 4xx (400/403/404/422) -> `provider_invalid_request`; 5xx -> `provider_unavailable` | Same mapping: 400/422 -> `provider_invalid_request`; 429 -> `provider_rate_limited`; 408/504 -> `provider_timeout`; 401/402 -> `provider_unavailable` (server-side credential/billing, never `provider_invalid_request`); 5xx -> `provider_unavailable` | Adapter-owned `openAIErrorCategory` / `deepSeekErrorCategory`; DeepSeek `TestCompleteChatClassifiesProviderError`, `TestCompleteChatClassifiesServerSideStatusesAsProviderUnavailable`, `TestStreamChatClassifiesServerSideStatusesBeforeStream`; OpenAI `TestCompleteChatClassifiesAuthenticationFailureAsProviderUnavailable`; `internal/dataplane:TestHandlerDeepSeekServerSideUpstreamStatusesAreNotClientErrors` |
...
> **Known inconsistency (deferred):** the non-stream path drops `reasoning_content` while the stream path
> passes it through. Resolving it means changing the public gateway API (adding a reasoning field to the
> `ChatResponse` envelope or adding thinking/reasoning request parameters) or normalizing the stream path, both
> public-contract changes requiring human approval. The Week 5/6 work deliberately does not change either path;
> it only records the asymmetry.
