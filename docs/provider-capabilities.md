...
| `system` messages | Sent inside `messages` with role `system` | Same as OpenAI | Only **leading** system messages (before the first user/assistant message) are translated into the top-level `system` field, joined with `"\n\n"`; `messages` then carry only `user`/`assistant`. A system message that appears after the first user/assistant turn cannot be expressed by the Messages API without silently reordering it, so the adapter rejects it (`unsupported_feature`) before any upstream HTTP work rather than changing conversation semantics |
